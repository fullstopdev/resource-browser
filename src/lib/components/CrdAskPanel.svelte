<script lang="ts">
	...
	async function submit() {
		...
		const result = await askAI(trimmed, crdContext);
		loading = false;
...
<div class="space-y-4">
	<div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-900/50">
		...
		<p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
			Answers use the open CRD spec/status schema as context. Powered by Cloudflare Workers AI.
		</p>
	</div>
